...
from main import app, workflow_costs, KILL_THRESHOLD, INPUT_COST_PER_TOKEN, OUTPUT_COST_PER_TOKEN
import pytest
...
def test_zero_token_event_does_not_trigger_kill():
    response = send_event("test-trace-1", tokens_in=0, tokens_out=0)
...
def test_different_trace_ids_are_isolated():
    ...
    response = send_event("trace-A", step=0, tokens_in=tokens_needed, tokens_out=0)
    assert response.json()["status"] == "kill"

    response = send_event("trace-B", step=0, tokens_in=100, tokens_out=0)
...
def test_output_tokens_priced_correctly():
    ...
    assert response.json()["status"] == "kill", "Output tokens alone should be able to trigger kill at the correct threshold"